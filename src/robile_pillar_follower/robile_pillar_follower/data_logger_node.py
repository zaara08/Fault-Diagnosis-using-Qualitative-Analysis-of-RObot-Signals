...
class DataLoggerNode(Node):
    """
    Data logger for pillar wall following.
    Saves to ~/rnd_ws/data_pillar/ folder.
    Includes all sensors + action labels.
    """
    ...
    def __init__(self):
        ...
        data_dir  = os.path.join(
            home_dir, 'rnd_ws', 'data_pillar')
        os.makedirs(data_dir, exist_ok=True)

        filename = os.path.join(
            data_dir, f'pillar_data_{timestamp}.csv')
        ...
        # Start position for distance tracking
        self.start_x           = None
        self.start_y           = None
        ...
        self.get_logger().info('Pillar Data Logger started!')
    ...
    def save_row(self):
        """Saves one row every 0.1 seconds."""
    ...
    def destroy_node(self):
        ...
        self.csv_file.close()
        self.get_logger().info(
            f'CSV saved! Total rows: {self.row_count}')
        super().destroy_node()
